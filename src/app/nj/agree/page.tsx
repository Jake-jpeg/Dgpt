    ...
    <div className="min-h-screen bg-zinc-50">
          ...
          <div className="flex h-16 items-center">
            <Link href="/nj" className="flex items-center gap-3 transition hover:opacity-80">
              ...
              <div>
                ...
                <p className="text-xs text-zinc-500">New Jersey — Review & Agree</p>
              </div>
            </Link>
          </div>
        </div>
      ...
      <main className="mx-auto max-w-2xl px-4 py-12 sm:px-6 lg:px-8">
        <div className="text-center mb-8">
          ...
          <p className="mt-2 text-zinc-600">Review the documents below, then confirm each statement to continue.</p>
        </div>

        <div className="mb-8 rounded-xl bg-blue-50 p-4 ring-1 ring-blue-200">
          <div className="space-y-1 text-sm text-blue-800">
            {Object.entries(courtesyNotes).map(([lang, note]) => (
              <p key={lang} className={lang === 'en' ? 'font-medium' : 'text-blue-700'}>{note}</p>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  ...
                  {hasReadTerms && <span className="inline-flex items-center rounded-full bg-green-100 px-2 py-0.5 text-xs font-medium text-green-700">✓ Opened</span>}
                </div>
                <p className="mt-1 text-sm text-zinc-500">Understand what DivorceGPT is (and isn't), eligibility requirements, and important disclaimers.</p>
              </div>
              <a href="/terms?review=true" target="_blank" rel="noopener noreferrer" onClick={handleTermsClick} className="flex-shrink-0 rounded-full bg-[#1a365d] px-4 py-2 text-sm font-medium text-white transition hover:bg-[#2c5282]">Read →</a>
            </div>
          </div>
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  ...
                  {hasReadPrivacy && <span className="inline-flex items-center rounded-full bg-green-100 px-2 py-0.5 text-xs font-medium text-green-700">✓ Opened</span>}
                </div>
                <p className="mt-1 text-sm text-zinc-500">Learn how we handle (and don't store) your data.</p>
              </div>
              <a href="/privacy?review=true" target="_blank" rel="noopener noreferrer" onClick={handlePrivacyClick} className="flex-shrink-0 rounded-full bg-[#1a365d] px-4 py-2 text-sm font-medium text-white transition hover:bg-[#2c5282]">Read →</a>
            </div>
          </div>

          <div className={`rounded-2xl p-6 ring-1 transition-all duration-200 ${allConfirmed ? 'bg-green-50 ring-green-300' : 'bg-white ring-zinc-200'}`}>
            <h3 className="font-semibold text-zinc-900 mb-4">Please confirm each statement:</h3>
            <div className="space-y-4">
              <label className="flex items-start gap-3 cursor-pointer">
                <input type="checkbox" checked={confirmNotLawFirm} onChange={(e) => setConfirmNotLawFirm(e.target.checked)} className="mt-1 h-4 w-4 flex-shrink-0 rounded border-zinc-300 text-[#1a365d] focus:ring-[#1a365d]" />
                <span className="text-sm text-zinc-700">I understand and agree that DivorceGPT is <strong>not a law firm</strong>, does not provide legal advice, and that my use of this service <strong>does not create an attorney-client relationship</strong></span>
              </label>
              <label className="flex items-start gap-3 cursor-pointer">
                <input type="checkbox" checked={confirmNoDataStored} onChange={(e) => setConfirmNoDataStored(e.target.checked)} className="mt-1 h-4 w-4 flex-shrink-0 rounded border-zinc-300 text-[#1a365d] focus:ring-[#1a365d]" />
                <span className="text-sm text-zinc-700">I understand my data is <strong>not stored</strong> — I am responsible for saving my documents</span>
              </label>
              <label className="flex items-start gap-3 cursor-pointer">
                <input type="checkbox" checked={confirmTermination} onChange={(e) => setConfirmTermination(e.target.checked)} className="mt-1 h-4 w-4 flex-shrink-0 rounded border-zinc-300 text-[#1a365d] focus:ring-[#1a365d]" />
                <span className="text-sm text-zinc-700">I understand threats or fraud will result in <strong>immediate session termination</strong></span>
              </label>
              <label className="flex items-start gap-3 cursor-pointer">
                <input type="checkbox" checked={confirmReadDocs} onChange={(e) => setConfirmReadDocs(e.target.checked)} className="mt-1 h-4 w-4 flex-shrink-0 rounded border-zinc-300 text-[#1a365d] focus:ring-[#1a365d]" />
                <span className="text-sm text-zinc-700">I have read the <strong>Terms of Service</strong> and <strong>Privacy Policy</strong></span>
              </label>
            </div>
          </div>
          ...
          <div className="text-center pt-4">
            <div className="mb-4">
              ...
              <p className="text-xs text-zinc-400 mt-2">After payment, you'll be redirected to your session. Bookmark the page — that URL is how you return.</p>
            </div>
            
            <div className="mb-4">
              <input type="text" value={freeKey} onChange={(e) => setFreeKey(e.target.value)} placeholder="Have an access code? Enter it here" className="w-full rounded-lg border border-zinc-200 px-4 py-2 text-sm text-center placeholder:text-zinc-400 focus:border-[#1a365d] focus:outline-none focus:ring-1 focus:ring-[#1a365d]" />
            </div>
            ...
            {!canProceed && (
              <p className="mt-3 text-sm text-zinc-500">
                {(!hasReadTerms || !hasReadPrivacy) ? "Please open and read both documents above" : "Please confirm all statements above"}
              </p>
            )}
          ...
          <div className="text-center pt-4">
            <Link href="/nj/qualify" className="inline-flex items-center gap-2 text-sm text-zinc-500 hover:text-zinc-700">
...
export default function NJAgreePage() {
  return (
    <Suspense fallback={<div className="flex min-h-screen items-center justify-center bg-zinc-50"><div className="h-12 w-12 animate-spin rounded-full border-4 border-[#1a365d] border-t-transparent" /></div>}>
      <AgreeContent />
    </Suspense>
  );
}
