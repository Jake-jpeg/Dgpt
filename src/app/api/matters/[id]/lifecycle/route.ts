...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (parsed.data.lifecycle !== undefined) {
      setMatterLifecycle(matter.id, parsed.data.lifecycle);
      recordAudit(
        matter.id,
    ...
    if (parsed.data.legalHold !== undefined) {
      setLegalHold(matter.id, parsed.data.legalHold, parsed.data.legalHoldReason);
      recordAudit(
        matter.id,
    ...
    const updated = getMatter(matter.id)!;
