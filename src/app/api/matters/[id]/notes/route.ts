...
export async function GET(req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    return Response.json({ notes: listInternalNotes(matter.id) });
  } catch (e) {
...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const target = listInternalNotes(matter.id).find((n) => n.id === parsed.data.noteId);
    if (!target) throw new HttpError(404, "Note not found");
    resolveInternalNote(target.id);
